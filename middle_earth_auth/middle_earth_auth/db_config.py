...
def dev_or_prod_db(debug, base_directory):
    if debug == 1:
        ...
        POSTGRES_DB = secret_key.get_secret_value('auth_postgres_db')
        POSTGRES_USER = secret_key.get_secret_value('auth_postgres_user')
        POSTGRES_PASSWORD = secret_key.get_secret_value('auth_postgres_password')
